<?php session_start();
    ...
    if(isset($_GET["fetch_data"])){
        ...
        if($_POST["search"] !=""){
            $query.=" WHERE unite_name LIKE '%".$_POST["search"]."%' OR unite_code LIKE '%".$_POST["search"]."%'  ";
        }else{
            $query.="";
        }
        
        if($_POST["orderby"] !=""){
            $query.=" ORDER BY unite_code ".$_POST["orderby"]." ";
        }else{
            $query.="";
        }
...
?>
            <tr class="table_hover" ondblclick="edit_function('<?php echo $row_sql['unite_code'];?>','<?php echo $row_sql['unite_name'];?>')">
                <td align="center"><?php echo $total_id++;?></td>
                <td><?php echo $row_sql["unite_name"]?></td>
                <td align="center">
                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="edit_function('<?php echo $row_sql['unite_code'];?>','<?php echo $row_sql['unite_name'];?>')">
                        <i class="fas fa-pen"></i> ແກ້ໄຂ
                    </button>
                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="delete_data('<?php echo $row_sql['unite_code'];?>','service-unite.php?delete','','','','','service-unite.php?fetch_data','display')">
...
<?php }
    if(isset($_GET["insert_data"])){
        ...
        if($_POST["unite_code"] !=""){
            $sql="unite_name='".$_POST["unite_name"]."' WHERE unite_code='".$_POST["unite_code"]."'";
            $edit=$db->fn_edit("res_unite",$sql);
            if($edit){
    ...
    if(isset($_GET["delete"])){
        $delete=$db->fn_delete("res_unite WHERE unite_code='".$_POST["field_id"]."' ");
        if($delete){
         echo json_encode(array("statusCode"=>200));
         }else{
