<?php session_start();
    ...
    if(isset($_GET["fetch_data"])){
        ...
        @$query.="res_category AS A LEFT JOIN res_group AS B ON A.cate_group=B.group_code ";

        if($_POST["search"] !=""){
            $query.=" WHERE group_name LIKE '%".$_POST["search"]."%' OR cate_name LIKE '%".$_POST["search"]."%'  ";
        }else{
            $query.="";
        }
        
        if($_POST["orderby"] !=""){
            $query.=" ORDER BY cate_code ".$_POST["orderby"]." ";
        }else{
            $query.="";
        }
...
?>
            <tr class="table_hover" ondblclick="edit_function('<?php echo $row_sql['cate_code']?>','<?php echo $row_sql['cate_name']?>','<?php echo $row_sql['cate_group']?>')">
                <td align="center"><?php echo $total_id++;?></td>
                <td><?php echo $row_sql["cate_name"]?></td>
                <td><?php echo $row_sql["group_name"]?></td>
                <td align="center">
                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="edit_function('<?php echo $row_sql['cate_code']?>','<?php echo $row_sql['cate_name']?>','<?php echo $row_sql['cate_group']?>')">
                        <i class="fas fa-pen"></i> ແກ້ໄຂ
                    </button>
                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="delete_data('<?php echo $row_sql['cate_code'];?>','service-category.php?delete','','','','','service-category.php?fetch_data','display')">
...
<?php }
    if(isset($_GET["insert_data"])){
        $auto_number=$db->fn_autonumber("cate_code","res_category");
        if($_POST["cate_code"] !=""){
            $sql="cate_name='".$_POST["cate_name"]."',cate_group='".$_POST["product_group_fk"]."' WHERE cate_code='".$_POST["cate_code"]."'";
            $edit=$db->fn_edit("res_category",$sql);
            if($edit){
            ...
            $sql="'".$auto_number."','".$_POST["cate_name"]."','".$_POST["product_group_fk"]."'";
            $insert=$db->fn_insert("res_category",$sql);
            if($insert){
    ...
    if(isset($_GET["delete"])){
        $delete=$db->fn_delete("res_category WHERE cate_code='".$_POST["field_id"]."' ");
        if($delete){
         echo json_encode(array("statusCode"=>200));
         }else{
