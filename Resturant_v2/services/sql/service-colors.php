<?php session_start();
    ...
    if(isset($_GET["fetch_data"])){
        ...
        if($_POST["search"] !=""){
            $query.=" WHERE color_name LIKE '%".$_POST["search"]."%' OR color_code LIKE '%".$_POST["search"]."%'  ";
        }else{
            $query.="";
        }
        
        if($_POST["orderby"] !=""){
            $query.=" ORDER BY color_code ".$_POST["orderby"]." ";
        }else{
            $query.="";
        }
...
?>
            <tr class="table_hover" ondblclick="edit_function('<?php echo $row_sql['color_code'];?>','<?php echo $row_sql['color_name'];?>')">
                <td align="center"><?php echo $total_id++;?></td>
                <td><?php echo $row_sql["color_name"]?></td>
                <td align="center">
                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="edit_function('<?php echo $row_sql['color_code'];?>','<?php echo $row_sql['color_name'];?>')">
                        <i class="fas fa-pen"></i> ແກ້ໄຂ
                    </button>
                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="delete_data('<?php echo $row_sql['color_code'];?>','service-colors.php?delete','','','','','service-colors.php?fetch_data','display')">
...
<?php }
    if(isset($_GET["insert_data"])){
        $auto_number=$db->fn_autonumber("color_code","res_colors");
        if($_POST["color_code"] !=""){
            $sql="color_name='".$_POST["color_name"]."' WHERE color_code='".$_POST["color_code"]."'";
            $edit=$db->fn_edit("res_colors",$sql);
            if($edit){
            ...
            $sql="'".$auto_number."','".$_POST["color_name"]."'";
            $insert=$db->fn_insert("res_colors",$sql);
            if($insert){
    ...
    if(isset($_GET["delete"])){
        $delete=$db->fn_delete("res_colors WHERE color_code='".$_POST["field_id"]."' ");
        if($delete){
         echo json_encode(array("statusCode"=>200));
         }else{
