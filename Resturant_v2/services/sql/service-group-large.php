<?php session_start();
    ...
    if(isset($_GET["fetch_data"])){
        ...
        if($_POST["search"] !=""){
            $query.=" WHERE group_name LIKE '%".$_POST["search"]."%' OR group_code LIKE '%".$_POST["search"]."%'  ";
        }else{
            $query.="";
        }
        
        if($_POST["orderby"] !=""){
            $query.=" ORDER BY group_code ".$_POST["orderby"]." ";
        }else{
            $query.="";
        }
...
?>
            <tr class="table_hover" ondblclick="edit_function('<?php echo $row_sql['group_code'];?>','<?php echo $row_sql['group_name'];?>')">
                <td align="center"><?php echo $total_id++;?></td>
                <td><?php echo $row_sql["group_name"]?></td>
                <td align="center">
                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="edit_function('<?php echo $row_sql['group_code'];?>','<?php echo $row_sql['group_name'];?>')">
                        <i class="fas fa-pen"></i> ແກ້ໄຂ
                    </button>
                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="delete_data('<?php echo $row_sql['group_code'];?>','service-group-large.php?delete','','','','','service-group-large.php?fetch_data','display')">
...
<?php }
    if(isset($_GET["insert_data"])){
        $auto_number=$db->fn_autonumber("group_code","res_group");
        if($_POST["group_code"] !=""){
            $sql="group_name='".$_POST["group_name"]."' WHERE group_code='".$_POST["group_code"]."'";
            $edit=$db->fn_edit("res_group",$sql);
            if($edit){
            ...
            $sql="'".$auto_number."','".$_POST["group_name"]."'";
            $insert=$db->fn_insert("res_group",$sql);
            if($insert){
    ...
    if(isset($_GET["delete"])){
        $delete=$db->fn_delete("res_group WHERE group_code='".$_POST["field_id"]."' ");
        if($delete){
         echo json_encode(array("statusCode"=>200));
         }else{
